...
def test_adp_follow_picks_lowest_adp() -> None:
    ...
    assert vols_dissent(payload, proposal).outcome is GateOutcome.PASS
    assert ecr_dissent(payload, proposal).outcome is GateOutcome.PASS
    # A dissenting baseline names the hint/ecr_best player mechanically, with
    # the label SYSTEM asks the model for, so it clears the eval-only
    # contains-floor (#20) without reasoning.
    assert why_contains_floor(payload, proposal).outcome is GateOutcome.PASS
    assert "VOLS pick" in proposal.why
    assert "ECR pick" in proposal.why
...
def test_ecr_follow_picks_lowest_ecr_across_positions() -> None:
    ...
    assert ecr_dissent(payload, proposal).outcome is GateOutcome.PASS
    assert why_contains_floor(payload, proposal).outcome is GateOutcome.PASS
    # Only the flag that fired gets its label. ecr_follow took ecr_best, so
    # it must not claim an ECR dissent it did not make.
    assert "VOLS pick" in proposal.why
    assert "ECR pick" not in proposal.why
